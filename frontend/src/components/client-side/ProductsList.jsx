...
export default function ProductsList() {
  ...
  const handleAddToCart = () => {
    handleAddProduct(quickView.product, quantity);
    setQuickView({ open: false, product: {} });
  };
  ...
  return (
    <>
        ...
        <div className='z-20 fixed pt-[430px] top-0 md:bottom-0 md:left-0 overflow-y-scroll h-full md:right-0 bg-black bg-opacity-35 flex items-center justify-center'>
          <div className='flex flex-col md:flex-row justify-between bg-white md:p-10 p-20 relative'>
            <IoMdClose
              ...
              onClick={() => setQuickView({ open: false, product: {} })}
            />
            <div>
              <img
                src={
              ...
              <div className='flex gap-2 mt-2'>
                {quickView.product.main_image && (
                  <img
                    ...
                    onClick={() => setSelectedImage(quickView.product.main_image)}
                    className={`w-[100px] h-[100px] object-cover border cursor-pointer border-black`}
                  />
                )}
                {quickView.product.second_image && (
                  <img
                    ...
                    onClick={() => setSelectedImage(quickView.product.second_image)}
                    className={'w-[100px] h-[100px] object-cover border cursor-pointer border-black'}
                  />
                )}
                {quickView.product.third_image && (
                  <img
                    ...
                    onClick={() => setSelectedImage(quickView.product.third_image)}
                    className={'w-[100px] h-[100px] object-cover border cursor-pointer border-black'}
                  />
                )}
              </div>
            ...
            <div className='p-6 flex flex-col'>
              ...
              <input
                ...
                className='border outline-none p-2 my-5 w-[50px]'
                min={1}
              />
              <p>{quickView.product.description}</p>
              <button
                onClick={handleAddToCart}
                className='bg-yellow-600 w-1/3 text-white p-2 rounded-md outline-none my-2'
              >
                Add to Cart
              </button>
      ...
      <h1 className='font-bold text-3xl text-center my-10 hover:text-yellow-600 hover:-translate-y-2 transition-all cursor-pointer'>Our Products</h1>
      {window.innerWidth < 700 ? (
        <div className='bg-gray-200 p-10 grid grid-cols-2 gap-5 my-20'>
          {products.map((product) => (
            <div key={product.id} className='product w-[200px] h-[210px] p-5 bg-white flex items-center justify-center flex-col'>
